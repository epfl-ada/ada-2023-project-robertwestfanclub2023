...
def process_continent_data(df_continent):
    """
    df_continent: continental dataset
    """
    winner = df_continent[df_continent["Winner"]== True]["Release Month"].value_counts()
    selection = df_continent["Release Month"].value_counts()
    ratio = winner / selection

    # Create a new Series with the complete set of months (1 to 12)
    complete_months = pd.Series(index=range(1, 13))

    # Fill missing months with 0
    ratio = ratio.combine_first(complete_months).fillna(0)
